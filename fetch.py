...
def extract_bedwars_important_stats(player_data):
    ...
    # Tokens (often a general player stat, not strictly Bedwars)
    ...
    bedwars_stats["Prestige"] = f"Determined by Bedwars Level ({bedwars_level})" # Prestige is implicit in level

    # Quests Completed (this can be tricky, looking for a general Bedwars quest stat)
    quests_completed = _get_nested_stat(player_data, "achievements.bedwars_quests_completed", 0)
    # Fallback for general quests if specific Bedwars quests aren't directly exposed this way
    if quests_completed == 0: # If the specific achievement key didn't work
        # Try finding a general count under 'quests' if available
        # Note: Hypixel API quest structure can be complex and vary
        pass # For now, stick to the known achievement key or default to 0
    ...
    # Resources Collected (not including dream modes)
...
def extract_skywars_important_stats(player_data):
    ...
    # Arrow Hit/Miss Ratio
    # The common interpretation is hit rate: hits / shots
